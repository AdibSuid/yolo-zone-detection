"""Configuration settings for YOLO detection system."""
import numpy as np


class PerformanceMode:
    """Performance mode configurations for different use cases."""
    
    ULTRA_FAST = {
        "name": "Ultra Fast",
        "model": "yolov8n_openvino_model/",
        "resolution": (320, 320),
        "frame_skip": 1,
        "conf_threshold": 0.6,
        "annotation_thickness": 1,
        "text_scale": 0.3
    }
    
    MAXIMUM_FPS = {
        "name": "Maximum FPS",
        "model": "yolov8n_openvino_model/",
        "resolution": (416, 416),
        "frame_skip": 1,
        "conf_threshold": 0.5,
        "annotation_thickness": 1,
        "text_scale": 0.4
    }
    
    BALANCED = {
        "name": "Balanced",
        "model": "yolov8s_openvino_model/",
        "resolution": (640, 480),
        "frame_skip": 2,
        "conf_threshold": 0.35,
        "annotation_thickness": 2,
        "text_scale": 0.5
    }
    
    HIGH_ACCURACY = {
        "name": "High Accuracy",
        "model": "yolov8s_openvino_model/",
        "resolution": (640, 480),
        "frame_skip": 1,
        "conf_threshold": 0.25,
        "annotation_thickness": 2,
        "text_scale": 0.5
    }
    
    # Custom YOLOv8 Model Configuration
    CUSTOM = {
        "name": "Custom YOLOv8",
        "model": "custom_yolov5_openvino_model/",  # Directory name (kept for compatibility)
        "resolution": (640, 640),  # Matches your training: image_size=640
        "frame_skip": 1,
        "conf_threshold": 0.5,  # Matches your config: confidence=0.5
        "iou_threshold": 0.5,   # Matches your config: iou_threshold=0.5 (NMS)
        "annotation_thickness": 2,
        "text_scale": 0.5
    }
    
    @classmethod
    def get_mode(cls, mode_name):
        """Get performance mode configuration.
        
        Args:
            mode_name: Name of the mode (ultra_fast, maximum_fps, balanced, high_accuracy, custom)
            
        Returns:
            Configuration dictionary
        """
        modes = {
            "ultra_fast": cls.ULTRA_FAST,
            "maximum_fps": cls.MAXIMUM_FPS,
            "balanced": cls.BALANCED,
            "high_accuracy": cls.HIGH_ACCURACY,
            "custom": cls.CUSTOM  # Add custom mode
        }
        return modes.get(mode_name, cls.BALANCED)
    
    @classmethod
    def list_modes(cls):
        """Print all available performance modes with their settings."""
        print("📋 Available Performance Modes:")
        print("=" * 60)
        modes = [
            ("ultra_fast", cls.ULTRA_FAST),
            ("maximum_fps", cls.MAXIMUM_FPS),
            ("balanced", cls.BALANCED),
            ("high_accuracy", cls.HIGH_ACCURACY),
            ("custom", cls.CUSTOM)  # Add custom mode
        ]
        for key, config in modes:
            print(f"\n🔧 {key}:")
            print(f"   📝 {config['name']}")
            print(f"   📐 Resolution: {config['resolution'][0]}x{config['resolution'][1]}")
            print(f"   ⏭️  Frame Skip: {config['frame_skip']}")
            print(f"   🎚️  Confidence: {config['conf_threshold']}")
            print(f"   🔧 Model: {config['model']}")


class CameraConfig:
    """Camera configuration settings."""
    
    # Auto-exposure settings (15 FPS, good brightness)
    AUTO_EXPOSURE_MODE = 0.75
    BRIGHTNESS = 128
    CONTRAST = 128
    BUFFER_SIZE = 1
    
    # Stabilization settings
    WARMUP_TIME = 1.0  # seconds
    WARMUP_FRAMES = 10
    
    # Reliability settings
    MAX_FAILED_READS = 10


class ZoneConfig:
    """Detection zone configuration."""
    
    # Box dimensions as fraction of frame size
    BOX_WIDTH_RATIO = 0.3  # 30% of frame width
    BOX_HEIGHT_RATIO = 0.4  # 40% of frame height
    
    @staticmethod
    def create_box_polygon(frame_width, frame_height):
        """Create centered box polygon for zone detection.
        
        Args:
            frame_width: Width of the frame
            frame_height: Height of the frame
            
        Returns:
            NumPy array of box corners [top-left, top-right, bottom-right, bottom-left]
        """
        box_width = int(frame_width * ZoneConfig.BOX_WIDTH_RATIO)
        box_height = int(frame_height * ZoneConfig.BOX_HEIGHT_RATIO)
        center_x = frame_width // 2
        center_y = frame_height // 2
        
        return np.array([
            [center_x - box_width // 2, center_y - box_height // 2],  # Top-left
            [center_x + box_width // 2, center_y - box_height // 2],  # Top-right
            [center_x + box_width // 2, center_y + box_height // 2],  # Bottom-right
            [center_x - box_width // 2, center_y + box_height // 2],  # Bottom-left
        ])


class MQTTConfig:
    """MQTT broker configuration."""
    
    DEFAULT_BROKER = "localhost"
    DEFAULT_PORT = 1883
    TOPIC = "cv/zone_events"
    
    @staticmethod
    def get_client_id(mode):
        """Generate MQTT client ID.
        
        Args:
            mode: Performance mode name
            
        Returns:
            Client ID string
        """
        return f"cv_publisher_{mode}"


class DisplayConfig:
    """Display window configuration."""
    
    SCALE_FACTOR = 1.5  # Window scaling for visibility
    WINDOW_NAME_TEMPLATE = "YOLO Zone Detection - {mode_name}"
